...
@section('title', 'Редактирование коньков')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-md p-6">
        <h2 class="text-xl font-bold text-gray-800 mb-6">Редактировать коньки</h2>

        <form action="{{ route('admin.skates.update', $skate) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="skate_model_id" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-blue-600" required>
                    @foreach($models as $model)
                        <option value="{{ $model->id }}" {{ $skate->skate_model_id == $model->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="size" value="{{ old('size', $skate->size) }}" min="35" max="48"
            ...
            <div class="mb-4">
                ...
                <input type="number" name="quantity" value="{{ old('quantity', $skate->quantity) }}" min="0" 
                       class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-blue-600" required>
                @error('quantity')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <input type="number" name="price_per_hour" value="{{ old('price_per_hour', $skate->price_per_hour) }}" min="0" step="1"
                       class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-blue-600" required>
